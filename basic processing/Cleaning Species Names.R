...
cleaned<-read.csv("habacuc.csv")%>%
  mutate(species=as.character(species),
         species_matched=as.character(species_matched))%>%
  mutate(match=ifelse(species==species_matched, 1,0))

write.csv(cleaned, "Species_to_check.csv", row.names=F)
# 
# splist<-splist_raw%>%
#   select(genus_species)%>%
#   unique()
...
#   filter(genus_species!="standing dead")%>%#this drops 70 lines of code in LUCERO
#   filter(genus_species!="nostoc sp.")%>%# this drops 1 line of code in MNT
#   unique()
# 
