...
t.test(CT_comp$PC_stab, mu=0) # yes overall sig
...
PC_bargraph<-rbind(PC_bargraph_all,PC_bargraph_trt )

##convert to box plots and put points in as well.
stab_fig<-
  ggplot(data=PC_bargraph, aes(x=trt_type6, y=stab, fill=trt_type6))+
...
sd_fig<-
  ggplot(data=PC_bargraph, aes(x=trt_type6, y=sd, fill=trt_type6))+
  geom_bar(position=position_dodge(), stat="identity")+
...
grid.arrange(stab_fig, sd_fig, mn_fig, ncol=1)
