...
keep <- c('label', 'expt.year', 'dist')
disp <- subset(all, mean.disp=='disp')
disp <- disp[,colnames(disp) %in% keep]
dispMean <- ddply(disp, c('label', 'expt.year'), summarise,
...
allClean <- merge(dispMean, mean, by=c('label', 'expt.year'), all=T)
names(allClean)[names(allClean)=="dist"] <- "mean_change"
names(allClean)[names(allClean)=="plot_mani.x"] <- "plot_mani"
drop <- c('X', 'plot_ID', 'plot_mani.y', 'dist.1', 'dist.2', 'mean.disp', 'dg')
