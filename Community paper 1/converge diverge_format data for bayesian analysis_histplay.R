...
m<-qplot(mean_change, data=divCompareExp, geom="histogram")+
...
s1<-qplot(S_PC, data=divCompareExp, geom="histogram")+
  ...
  xlab("Proportion Change in Richness")+
  geom_vline(xintercept = 0, size=2)
...
e2<-qplot(SimpEven_change, data=divCompareExp, geom="histogram")+
...
divCompareExpMaxN <- merge(divCompare, expInfo, by=c('exp_year', 'treatment', 'plot_mani'))%>%
...
divCompareExp<- merge(divCompare, expInfo, by=c('exp_year', 'treatment', 'plot_mani'))%>%
  #removing treatments that were pulses, did not directly manipulate a resource, or had ceased and pre-treatment data
  filter(pulse==0, resource_mani==1, treatment_year>0)%>%
  select(exp_year, treatment, plot_mani, mean_change, dispersion_change, H_change, S_PC, SimpEven_change, site_code, project_name, community_type, calendar_year,n)

divCompareExpNOCDR<- merge(divCompare, expInfo, by=c('exp_year', 'treatment', 'plot_mani'))%>%
  #removing treatments that were pulses, did not directly manipulate a resource, or had ceased and pre-treatment data
  filter(pulse==0, resource_mani==1, treatment_year>0, site_code!="CDR")%>%
...
divCompareMax<-merge(divCompareExpMaxN, divCompareExp, by=c("site_code","project_name","community_type","n"))
