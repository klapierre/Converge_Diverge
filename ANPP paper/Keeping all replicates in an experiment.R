...
t<-t.test(CT_comp$PD_CV, mu=0) # overall No, and not for the difference GCDs
...
# # nuts temporal
subdat<-subset(subset(sub, trt_type6=="Multiple Nutrients"))
dat<-subdat[,c(3,6)]
...
sub.mn<-subset(CT_all, sig_mean!=0)
mndat<-sub.mn[,c(4,5)]
...
cv1fig<-
  ggplot(data=sub, aes(x=cont_temp_cv, y=anpp_temp_cv, color = trt_type7))+
  geom_point(size=3)+
