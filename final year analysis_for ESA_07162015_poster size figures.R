...
theme_set(theme_bw())
theme_update(axis.title.x=element_text(size=38, vjust=-0.35), axis.text.x=element_text(size=28),
             axis.title.y=element_text(size=38, angle=90, vjust=0.5), axis.text.y=element_text(size=28),
             plot.title = element_text(size=44, vjust=2),
...
meanManiPlot <- ggplot(barGraphStats(data=meanManiType, variable="dist_sqrt", byFactorNames=c("mani_type")), aes(x=mani_type, y=mean)) +
  ...
  annotate('text', x=1 ,y=0.57, label='ab', size=10) +
  annotate('text', x=2 ,y=0.55, label='a', size=10) +
  annotate('text', x=3 ,y=0.58, label='b', size=10) +
  annotate('text', x=0.6, y=0.57, label='(a)', size=10)
summary(meansMAP <- lm(MAP.x~dist_RR, subset(meanManiType, dist_sqrt<3)))
meanMAPplot <- ggplot(subset(meanManiType, dist_sqrt<3), aes(x=MAP.x, y=dist_sqrt)) +
  geom_point(shape=1) +
  scale_x_continuous(name="Mean Annual Precipitation") +
  scale_y_continuous(breaks=seq(0,1,0.1), name=element_blank()) +
  coord_cartesian(ylim=c(0,1)) +
  annotate('text', x=200, y=0.95, label='(b)', size=10)
summary(meansANPP <- lm(ANPP.x~dist_sqrt, subset(meanManiType, dist_sqrt<3)))
meansANPPText <- grobTree(textGrob(expression(paste(R^2, " = 0.092, p < 0.001", sep="")), x=1, y=0.05, hjust=0, gp=gpar(col="black", fontsize=15)))
meanANPPplot <- ggplot(subset(meanManiType, dist_sqrt<3), aes(x=ANPP.x, y=dist_sqrt)) +
  geom_point(shape=1) +
  ...
  annotate('text', x=5, y=0.95, label='(c)', size=10) +
  annotate('text', x=5, y=0.1, label='R  = 0.092, p < 0.001', size=8, hjust=0)
summary(meansEcosystem <- aov(dist_sqrt~broad.ecosystem.type.y, subset(meanManiType, dist_sqrt<3)))
TukeyHSD(meansEcosystem)
meanEcosystemPlot <- ggplot(barGraphStats(data=subset(meanManiType, dist_sqrt<3), variable="dist_sqrt", byFactorNames=c("broad.ecosystem.type.y")), aes(x=broad.ecosystem.type.y, y=mean)) +
  geom_bar(stat="identity", fill='white', colour='black') +
  geom_errorbar(aes(ymin=mean-(se), ymax=mean+(se), width=0.2)) +
  scale_y_continuous(name="Change in Mean", breaks=seq(0,0.6,0.2)) +
  coord_cartesian(ylim=c(0,0.6)) +
  ...
  annotate('text', x=2 ,y=0.57, label='a', size=7) +
  annotate('text', x=3 ,y=0.40, label='b', size=7) +
  annotate('text', x=4 ,y=0.53, label='a', size=7) +
  annotate('text', x=5 ,y=0.55, label='ab', size=7) +
  annotate('text', x=0.8, y=0.57, label='(d)', size=10)
summary(meansSpp <- lm(species_num.x~dist_sqrt, subset(meanManiType, dist_sqrt<3)))
meanSppPlot <- ggplot(subset(meanManiType, dist_sqrt<3), aes(x=species_num.x, y=dist_sqrt)) +
  geom_point(shape=1) +
  scale_x_continuous(breaks=seq(0,185,50), name="Gamma Diversity") +
  scale_y_continuous(breaks=seq(0,1,0.1), name=element_blank()) +
  coord_cartesian(ylim=c(0,1)) +
  annotate('text', x=10, y=0.95, label='(e)', size=10)
...
dispManiPlot <- ggplot(barGraphStats(data=subset(dispersionManiType, dist_RR<3), variable="dist_RR", byFactorNames=c("mani_type", "con_div")), aes(x=mani_type, y=mean, fill=con_div)) +
  ...
  annotate('text', x=1 ,y=0.18, label='a', size=10) +
  annotate('text', x=2 ,y=0.31, label='b', size=10) +
  annotate('text', x=3 ,y=0.28, label='b', size=10) +
  theme(legend.position='none') +
  annotate('text', x=0.6, y=0.3, label='(a)', size=10)
...
dispANPPplot <- ggplot(subset(dispersionManiType, dist_RR<3), aes(x=ANPP, y=dist_RR, colour=con_div)) +
  geom_point(shape=1) +
#   geom_smooth(aes(y=dist_RR, x=ANPP), data=subset(dispersionManiType, dist_RR<3 & con_div=='con'), method=lm, se=T, colour="#339900") +
  scale_x_continuous(breaks=seq(0,1000,200), name="ANPP") +
  scale_y_continuous(name=element_blank()) +
  #coord_cartesian(ylim=c(0,1)) +
  scale_colour_manual(values=color) +
  ...
  annotation_custom(meansANPPText) +
  annotate('text', x=5, y=0.95, label='(c)', size=10) #+
#   annotate('text', x=5, y=-0.52, label='R  = 0.540, p = 0.012', size=8, hjust=0)
...
dispEcosystemPlot <- ggplot(barGraphStats(data=subset(dispersionManiType, dist_RR<3), variable="dist_RR", byFactorNames=c("broad.ecosystem.type", "con_div")), aes(x=broad.ecosystem.type, y=mean, fill=con_div)) +
  ...
  scale_fill_manual(values=color) +
  theme(legend.position='none') +
  annotate('text', x=0.8, y=0.45, label='(d)', size=10)
