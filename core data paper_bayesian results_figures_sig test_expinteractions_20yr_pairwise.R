...
rawData <- read.csv('ForAnalysis_allAnalysis20yr_pairwise.csv')
...
rawData2<- rawData%>%
  ...
  summarise(mean_mean=mean(BC_between_diff), std_mean=sd(BC_between_diff), mean_rich=mean(S_PC), std_rich=sd(S_PC)) #to backtransform
...
# chains1 <- read.csv('C:\\Users\\lapie\\Dropbox (Smithsonian)\\working groups\\converge diverge working group\\converge_diverge\\La Pierre_comm difference_final model results_01122018\\20 yr_pairwise\\simple_mod_effs_coding_interactions_20yrpairwise_0.csv', comment.char='#')
# chains1 <- chains1[-1:-5000,]
# chains2 <- read.csv('C:\\Users\\lapie\\Dropbox (Smithsonian)\\working groups\\converge diverge working group\\converge_diverge\\La Pierre_comm difference_final model results_01122018\\20 yr_pairwise\\simple_mod_effs_coding_interactions_20yrpairwise_1.csv', comment.char='#')
# chains2 <- chains2[-1:-5000,]
# chains3 <- read.csv('C:\\Users\\lapie\\Dropbox (Smithsonian)\\working groups\\converge diverge working group\\converge_diverge\\La Pierre_comm difference_final model results_01122018\\20 yr_pairwise\\simple_mod_effs_coding_interactions_20yrpairwise_2.csv', comment.char='#')
# chains3 <- chains3[-1:-5000,]
# chains4 <- read.csv('C:\\Users\\lapie\\Dropbox (Smithsonian)\\working groups\\converge diverge working group\\converge_diverge\\La Pierre_comm difference_final model results_01122018\\20 yr_pairwise\\simple_mod_effs_coding_interactions_20yrpairwise_3.csv', comment.char='#')
...
# 
# write.csv(chainsCommunity2, 'bayesian_output_summary_expinteractions_20yr_pairwise_03182018.csv')

chainsCommunity2 <- read.csv('bayesian_output_summary_expinteractions_20yr_pairwise_03182018.csv')
...
# chainsFinalMean <- as.data.frame(colMeans(chainsCommunity[,2798:4669]))%>% #may need to delete original four chains dataframes to get this to work
#   add_rownames('parameter')
# names(chainsFinalMean)[names(chainsFinalMean) == 'colMeans(chainsCommunity[, 2798:4669])'] <- 'mean'
# #get sd of parameter values across all runs for each experiment, treatment, etc
# chainsFinalSD <- as.data.frame(colSd(chainsCommunity[,2798:4669]))
# names(chainsFinalSD)[names(chainsFinalSD) == 'colSd(chainsCommunity[, 2798:4669])'] <- 'sd'
# 
...
# 
# write.csv(chainsFinal, 'bayesian_output_mean sd_expinteractions_20yr_pairwise_01122018.csv')

chainsFinal <- read.csv('bayesian_output_mean sd_expinteractions_20yr_pairwise_01122018.csv')
...
chainsEquations <- chainsExperiment%>%
  ...
  mutate(yr9=ifelse(variable=='mean', (intercept+linear*7+quadratic*7^2)*(0.1537852)+(0.4981682), (intercept+linear*7+quadratic*7^2)*(0.1998256)+(-0.02436018)))%>%
  mutate(yr_final=ifelse(variable=='mean', (intercept+linear*alt_length+quadratic*alt_length^2)*(0.1537852)+(0.4981682),
                         (intercept+linear*alt_length+quadratic*alt_length^2)*(0.1998256)+(-0.02436018)))%>%
  ...
  mutate(curve1='stat_function(fun=function(x){(',
         curve2=' + ',
         curve3='*x + ',
         curve4=ifelse(variable=='mean', '*x^2)*(0.1537852)+(0.4981682)}, size=2, xlim=c(0,',
                       '*x^2)*(0.1998256)+(-0.02436018)}, size=2, xlim=c(0,'),
         curve5='), colour=',
         curve6=') +',
...
# write.csv(chainsEquations,'plot mani_equations_expinteractions_20yr_pairwise_01172018.csv', row.names=F)
...
# 
# write.csv(chainsCommunitySummary, 'bayesian_output_summary_final plots_expinteraction_20yr_pairwise_01122018.csv')

chainsCommunitySummary <- read.csv('bayesian_output_summary_final plots_expinteraction_20yr_pairwise_01122018.csv')
